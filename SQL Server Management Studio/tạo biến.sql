...
CREATE DATABASE taobien_K15full
GO 
USE taobien_K15full;
...
--hiển thị năm sinh trẻ nhất của giảng viên 
DECLARE @NamSinhTreNhat INT;
SET @NamSinhTreNhat = (SELECT MAX(namSinh) FROM GiangVien);
	
SELECT maGV, hoTenGV
FROM GiangVien
WHERE namSinh = @NamSinhTreNhat;

--hiển thị năm sinh lớn tuổi nhất của giảng viên 
DECLARE @NamSinhGiaNhat INT;
SET @NamSinhGiaNhat = (SELECT MIN(namSinh) FROM GiangVien);

SELECT maGV, hoTenGV
FROM GiangVien
WHERE namSinh = @NamSinhGiaNhat;

--3. CHAR lưu mã Khoa CNTT 
	--- Xuất DS Sinh Viên Khoa CNTT
	---Xuất DS Giảng Viên Khoa CNTT
	DECLARE @CNTT CHAR(25) ;
	SET  @CNTT = 'CNTT';
...
--4. NVARCHAR lưu tên khoa: 'Nội Thất'
	-- Xuất DS Sinh Viên 'Nội Thất'
	-- Xuất DS Giảng Viên 'Nội Thất'
DECLARE @MaKhoaNoiThat NVARCHAR(10);
SELECT @MaKhoaNoiThat = maKhoa
FROM Khoa
WHERE tenKhoa = N'Nội Thất';

SELECT *
FROM SinhVien
WHERE maKhoa = @MaKhoaNoiThat;

SELECT *
FROM GiangVien
WHERE maKhoa = @MaKhoaNoiThat;

--6. NVARCHAR lưu tên khoa: 'Xây Dựng'
	-- Xuất DS Sinh Viên Khoa 'Xây Dựng'
	-- Xuất DS Giảng Viên Khoa 'Xây Dựng'
DECLARE @MaKhoaXayDung CHAR(10);
SELECT @MaKhoaXayDung = 'XD'; -- Lưu trực tiếp mã khoa "Xây Dựng"

SELECT *
FROM SinhVien
WHERE maKhoa = @MaKhoaXayDung;
...
--7. NVARCHAR lưu Giới Tính: 'Nữ'
	-- Xuất DS Sinh Viên 'Nữ'
	-- Xuất DS Giảng Viên 'Nữ'
	DECLARE @GioiTinh NVARCHAR(5);
	SET @GioiTinh = N'Nữ';

SELECT maSV, hoTenSV, gioiTinh, namSinh, queQuan
FROM SinhVien
WHERE gioiTinh = @GioiTinh;

SELECT maGV, hoTenGV, gioiTinh, namSinh, diaChi
FROM GiangVien
WHERE gioiTinh = @GioiTinh;
--8. INT lưu năm sinh của SV nhỏ tuổi nhất
	-- In Thông Tin SV nhỏ Tuổi Nhất.
	DECLARE @NamSinhNhoNhat INT;
SELECT @NamSinhNhoNhat = MAX(namSinh) FROM SinhVien;

SELECT maSV, hoTenSV, gioiTinh, namSinh, queQuan
FROM SinhVien
WHERE namSinh = @NamSinhNhoNhat;
--9. INT lưu năm sinh của SV lớn tuổi nhất
	-- In Thông Tin SV lớn Tuổi Nhất.
	DECLARE @NamSinhToNhat INT;
SELECT @NamSinhToNhat = MIN(namSinh) FROM SinhVien;

SELECT maSV, hoTenSV, gioiTinh, namSinh, queQuan
FROM SinhVien
WHERE namSinh = @NamSinhToNhat;